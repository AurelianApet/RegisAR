...
public class OnlineMapsDrawingElement: IOnlineMapsInteractiveElement
{
    ...
    private static double[] GetBufferPoints(Vector2 bufferPosition, float zoom, IEnumerable points, out double minX, out double maxX, out double minY, out double maxY)
    {
        ...
        foreach (object p in points)
        {
            ...
            {
                double tx = 0, ty = 0;
                if (valueType == 1) map.projection.CoordinatesToTile((float) v2, (float) v1, izoom, out tx, out ty);
                else if (valueType == 2) map.projection.CoordinatesToTile((double) v2, (double) v1, izoom, out tx, out ty);
                tx = (tx - bufferPosition.x) * scaledTileSize;
                ty = (ty - bufferPosition.y) * scaledTileSize;
    ...
    public void OnRemoveFromMap()
    {
        if (materials != null)
        {
            foreach (Material material in materials) OnlineMapsUtils.Destroy(material);
            materials = null;
        }

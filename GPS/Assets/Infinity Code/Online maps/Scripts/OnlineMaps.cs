...
#region Variables
    ...
    /// </summary>
    public const string version = "3.0.0.33";
    ...
    private void LateUpdate()
    {
        ...
        if(markers.Length > 0)
        {
            markers[0].SetPosition(Global.curPos.longitude, Global.curPos.latitude);
    ...
    {
        double lng, lat;
        projection.TileToCoordinates(tx, ty, tileZoom ?? zoom, out lng, out lat);
        SetPosition(lng, lat);
    }
    ...
    private void Update()
    {
        if (Global.gps_set && !Global.map_set)
        {
            latitude = Global.curPos.latitude;
            longitude = Global.curPos.longitude;
            SetPosition(longitude, latitude);
            Global.map_set = true;
            OnlineMaps.instance.Redraw();
        }

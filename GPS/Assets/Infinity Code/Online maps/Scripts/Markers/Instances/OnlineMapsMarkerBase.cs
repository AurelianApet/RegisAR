...
public class OnlineMapsMarkerBase: IOnlineMapsInteractiveElement
{
    ...
    public static GameObject markerPanel;
    private void OnMarkerPress(OnlineMapsMarkerBase marker)
    {
        OnlineMapsControlBase.instance.dragMarker = this;
        Global.curMarkerLat = marker.position.y;
        Global.curMarkerLng = marker.position.x;
        if (markerPanel)
        {
            markerPanel.transform.Find("gps/InputField").GetComponent<InputField>().text = marker.position.y.ToString() + "," + marker.position.x.ToString();
        }
    }
